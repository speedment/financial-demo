 ...
 * | 0x0030 | 0x0033 | 0x04 |  trader_group_type | endpos |           String |
 * | 0x0034 | 0x0037 | 0x04 |    instrument_name | endpos |           String |
 * +--------+--------+------+--------------------+--------+------------------+
 * |                               Row Storage                               |
 * +--------+--------+------+--------------------+--------+------------------+
 * |                 Constant Nullables and Variable Fields                  |
 * +--------+--------+------+--------------------+--------+------------------+
 * | 0x0038 | 0x7FFE | 0x00 | Variable data area |   data |           byte[] |
 * +--------+--------+------+--------------------+--------+------------------+
 * </pre>
 * <p>
 * Variable data area layout order:
 * limitPrice, instrumentSymbol, instrumentSector, instrumentIndustry,
 * traderName, traderGroup, traderGroupType, instrumentName
 * 
 * @author Speedment
 */
@Generated("Speedment")
public class GeneratedOrderEntityStoreSerializerImpl implements EntityStoreSerializer<Order> {
    ...
    public StringSelection selectString(ByteBuffer in, int rowOffset, StringField<Order, ?> field) {
        ...
        if (colId instanceof Order.Identifier) {
            ...
            switch (_id) {
                ...
                case TRADER_GROUP_TYPE   : return selectStringTraderGroupType(in,      rowOffset, Math.abs(in.getInt(rowOffset + 44)), in.getInt(rowOffset + 48));
                case INSTRUMENT_NAME     : return selectStringInstrumentName(in,       rowOffset, Math.abs(in.getInt(rowOffset + 48)), in.getInt(rowOffset + 52));
    ...
    public <T> T deserializeReference(ByteBuffer in, int rowOffset, HasReferenceValue<Order, ?, T> field) {
        ...
        if (colId instanceof Order.Identifier) {
            ...
            switch (_id) {
                ...
                case TRADER_GROUP_TYPE   : return (T) deserializeTraderGroupType(in,      rowOffset, Math.abs(in.getInt(rowOffset + 44)), in.getInt(rowOffset + 48));
                case INSTRUMENT_NAME     : return (T) deserializeInstrumentName(in,       rowOffset, Math.abs(in.getInt(rowOffset + 48)), in.getInt(rowOffset + 52));
                default : throw new UnsupportedOperationException(
                    String.format("Unknown enum constant '%s'.", _id)
                );
            }
    ...
    public Order deserialize(ByteBuffer in, int rowOffset) {
        ...
        final int traderGroupTypeEnd    = in.getInt(rowOffset + 48);
        final int instrumentNameEnd     = in.getInt(rowOffset + 52);
    ...
    public int serialize(ByteBuffer out, int rowOffset, Order entity) throws BufferOverflowException {
        ...
        startPos = serializeTraderGroupType(out, rowOffset, startPos, 48, entity.getTraderGroupType());
        startPos = serializeInstrumentName(out, rowOffset, startPos, 52, OptionalUtil.unwrap(entity.getInstrumentName()));
        return startPos;
    }
    ...
    /**
     * Returns a selection of the trader_group_type of the orders.
     * 
     * @param in        the byte buffer to read from
     * @param rowOffset index in buffer where the row starts
     * @param begin     where in the row the field begins
    ...
    public boolean isNull(ByteBuffer in, int rowOffset, Field<Order> field) {
        ...
        if (colId instanceof Order.Identifier) {
            ...
            switch (_id) {
                ...
                case INSTRUMENT_NAME     : return in.getInt(rowOffset + 52) < 0;
                default : throw new UnsupportedOperationException(
                    String.format("Unknown enum constant '%s'.", _id)
                );
            }
